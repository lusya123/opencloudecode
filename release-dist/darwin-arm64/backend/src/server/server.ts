...
export namespace Server {
  ...
  export function url(): URL {
    return _url ?? new URL("http://localhost:4096/api")
  }
  ...
  export const App: () => Hono = lazy(() => {
    ...
    app
        ...
        .get(
          ...
          describeRoute({
            ...
            description: "Retrieve the global (user-level) OpenCode configuration.",
            operationId: "global.config.get",
            responses: {
              200: {
                description: "Global config info",
                content: {
                  "application/json": {
          ...
          async (c) => {
            return c.json(await Config.global())
          },
        )
        .patch(
          "/global/config",
          describeRoute({
            ...
            description: "Update the global (user-level) OpenCode configuration.",
        ...
        .route("/scheduler", SchedulerRoute)
        ...
        .all("/cc-switch/*", async (c) => {
          ...
          const ccSwitchIndex = requestPath.indexOf("/cc-switch")
          const suffix = ccSwitchIndex === -1 ? "" : requestPath.slice(ccSwitchIndex + "/cc-switch".length)
    ...
    const webRoot = process.env.OPENCODE_WEB_ROOT
    if (webRoot) {
      ...
      const serveIndex = () => {
        if (!fs.existsSync(indexPath)) {
          return new Response("OPENCODE_WEB_ROOT is set but index.html was not found", { status: 500 })
        }
      ...
      const handler = (c: any) => {
        const requestPath = c.req.path
        if (requestPath === "/" || requestPath === "") return serveIndex()
        ...
        if (fs.existsSync(candidate)) {
          ...
          if (stat.isFile()) return serveFile(candidate)
        }
  ...
  export function listen(opts: { port: number; hostname: string; mdns?: boolean; cors?: string[] }) {
    ...
    Scheduler.init().catch((e) => log.error("Failed to initialize scheduler", { error: e }))
    ...
    _url = new URL(server.url.toString())
    _url.pathname = "/api"
