  ...
  export async function environment() {
    ...
    return [
      [
        ...
        `</env>`,
        `<opencode>`,
        `  Scheduling: For recurring jobs, prefer the built-in scheduler tool (id: scheduler). Avoid OS cron/launchd via bash unless explicitly requested.`,
        `  定时任务：遇到“每天/每周/固定时间运行”等需求，优先使用 scheduler 工具创建/管理任务；除非用户明确要求，否则不要用 bash 去写 cron/launchd。`,
        `</opencode>`,
        `<files>`,
        `  ${
