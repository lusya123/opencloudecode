...
```
http://<hostname>:<port>/api/doc
```

For example, `http://localhost:4096/api/doc`. Use the spec to generate clients or inspect request and response types. Or view it in a Swagger explorer.

All API paths below are relative to the `/api` prefix.
