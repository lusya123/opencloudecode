...
function J(e, r$3 = {}) {
	const n = {
		...
		switch (o$2.parent = p$1, o$2.nextIndex++, u$1.type) {
			...
			case "CharacterClassOpen": return re$1(u$1, o$2, N);
...
var JavaScriptScanner = class {
	...
	findNextMatchSync(string, startPosition, _options) {
		...
		if (pending.length) {
			...
			for (const [i$2, match, offset] of pending) {
				if (match.index === minIndex) {
					return toResult(i$2, match, offset);
				}
			}
		}
		return null;
...
function resolveColorReplacements(theme, options) {
	...
	for (const [key$1, value] of Object.entries(options?.colorReplacements || {})) {
		if (typeof value === "string") replacements[key$1] = value;
...
function splitToken(token$1, offsets) {
	...
	if (lastOffset < token$1.content.length) {
		tokens.push({
			...token$1,
...
function flatTokenVariants(merged, variantsOrder, cssVariablePrefix, defaultColor, colorsRendering = "css-vars") {
	...
	styles.forEach((cur, idx) => {
		for (const key$1 of styleKeys) {
			...
			if (idx === 0 && defaultColor && COLOR_KEYS.includes(key$1)) {
...
function transformerDecorations() {
	...
	return {
		...
		tokens(tokens) {
			...
			const breakpoints = ctx.decorations.flatMap((d$1) => [d$1.start.offset, d$1.end.offset]);
		...
		code(codeEl) {
			...
			function applyLineSection(line, start, end, decoration) {
				...
				if (startIndex === -1 || endIndex === -1) {
					for (let i$2 = 0; i$2 < lineEl.children.length; i$2++) {
...
function createColorPalette(namedColorsMap = defaultNamedColorsMap) {
	...
	function getColorTable() {
		...
		colorTable = [];
		for (let i$2 = 0; i$2 < namedColors.length; i$2++) {
			colorTable.push(namedColor(namedColors[i$2]));
		}
		let levels = [
			0,
			95,
		...
		for (let r$3 = 0; r$3 < 6; r$3++) {
			for (let g = 0; g < 6; g++) {
				for (let b$2 = 0; b$2 < 6; b$2++) {
		...
		let level = 8;
		for (let i$2 = 0; i$2 < 24; i$2++, level += 10) {
			colorTable.push(rgbColor([
				level,
				level,
				level
...
function tokenizeAnsiWithTheme(theme, fileContents, options) {
	...
	const ansiPalette = Object.fromEntries(namedColors.map((name) => {
		...
		const themeColor = theme.colors?.[key$1];
...
function dimColor(color) {
	...
	if (hexMatch) {
		...
		if (hex.length === 8) {
			...
			const b$2 = hex[2];
			const a$1 = hex[3];
			const alpha = Math.round(Number.parseInt(`${a$1}${a$1}`, 16) / 2).toString(16).padStart(2, "0");
			return `#${r$3}${r$3}${g}${g}${b$2}${b$2}${alpha}`;
...
function codeToTokensWithThemes(internal, code, options) {
	...
	const tokens = syncThemesTokenization(...themedTokens.map((i$2) => i$2.tokens));
...
function tokensToHast(tokens, options, transformerContext, grammarState = getLastGrammarStateFromMap(tokens)) {
	...
	for (const [key$1, value] of Object.entries(options.meta || {})) {
		if (!key$1.startsWith("_")) properties[key$1] = value;
	}
	let preNode = {
		type: "element",
		tagName: "pre",
	...
	if (structure === "classic") {
		...
		for (const child of root$1.children) {
	...
	return tokens.map((line) => {
		...
		for (const token$1 of line) {
			...
			if (!isPrevDecorated && !isDecorated && prevStyle === currentStyle) {
				prevToken.content += token$1.content;
			} else {
				newLine.push({ ...token$1 });
			}
		}
...
function normalizeTheme(rawTheme) {
	...
	for (const key$1 of Object.keys(theme.colors || {})) {
		if (key$1 === "editor.foreground" || key$1 === "editor.background" || key$1.startsWith("terminal.ansi")) {
...
async function resolveThemes(themes) {
	...
	return resolved.filter((i$2) => !!i$2);
}
	...
	loadLanguages(langs) {
		...
		if (missingLangs.length) {
			const dependents = langsGraphArray.filter(([_$2, lang]) => {
				...
				return embedded?.some((l$2) => missingLangs.map(([name]) => name).includes(l$2));
...
function createBundledHighlighter(options) {
	...
	async function createHighlighter(options2) {
		...
		const _themes = (options2.themes ?? []).map((i$2) => resolveTheme(i$2));
		const langs = (options2.langs ?? []).map((i$2) => resolveLang(i$2));
	...
	for (const line of lines) {
		if (matchAlgorithm === "v3") {
			const splittedElements = line.children.flatMap((element$1, idx) => {
				...
				const isComment = matchToken(token$1.value, isLast);
				if (!isComment) return element$1;
				const rawSplits = token$1.value.split(/(\s+\/\/)/);
				if (rawSplits.length <= 1) return element$1;
				let splits = [rawSplits[0]];
				for (let i$2 = 1; i$2 < rawSplits.length; i$2 += 2) {
					splits.push(rawSplits[i$2] + (rawSplits[i$2 + 1] || ""));
				}
			...
			if (!match && i$2 > 0 && head$1.value.trim().startsWith("[!code")) {
				...
				if (prevToken?.type === "element") {
					...
					if (prevHead?.type === "text" && prevHead.value.includes("//")) {
						const combinedValue = prevHead.value + head$1.value;
...
function v1ClearEndCommentPrefix(text$1) {
	...
	if (match && match[1].trim().length === 0) {
		return text$1.slice(0, match.index);
	}
	return text$1;
}
	...
	return {
		...
		code(code) {
			...
			for (const comment$1 of parsed) {
				...
				if (isEmpty && comment$1.isLineCommentOnly) {
					linesToRemove.push(comment$1.line);
				} else if (isEmpty && comment$1.isJsxStyle) {
					comment$1.line.children.splice(comment$1.line.children.indexOf(comment$1.token) - 1, 3);
				} else if (isEmpty) {
					if (comment$1.additionalTokens) {
						for (let j$1 = comment$1.additionalTokens.length - 1; j$1 >= 0; j$1--) {
							const additionalToken = comment$1.additionalTokens[j$1];
							const tokenIndex = comment$1.line.children.indexOf(additionalToken);
							if (tokenIndex !== -1) {
								comment$1.line.children.splice(tokenIndex, 1);
							}
						}
					}
					...
					if (head$1.type === "text") {
						...
						if (comment$1.additionalTokens) {
							for (const additionalToken of comment$1.additionalTokens) {
...
function parseMetaHighlightString(meta) {
	...
	const lines = match[1].split(",").flatMap((v$1) => {
...
function parseMetaHighlightWords(meta) {
	...
	return match.map((v$1) => v$1[1].replace(/\\(.)/g, "$1"));
}
...
function transformerNotationMap(options = {}, name = "@shikijs/transformers:notation-map") {
	...
	return createCommentNotationTransformer(name, new RegExp(`#?\\s*\\[!code (${Object.keys(classMap).map(escapeRegExp).join("|")})(:\\d+)?\\]`, "gi"), function([_$2, match, range$1 = ":1"], _line, _comment, lines, index) {
		...
		for (let i$2 = index; i$2 < Math.min(index + lineNum, lines.length); i$2++) {
			this.addClassToHast(lines[i$2], classMap[match]);
		}
...
function transformerNotationWordHighlight(options = {}) {
	...
	return createCommentNotationTransformer("@shikijs/transformers:notation-highlight-word", /\s*\[!code word:((?:\\.|[^:\]])+)(:\d+)?\]/, function([_$2, word, range$1], _line, comment$1, lines, index) {
		...
		for (let i$2 = index; i$2 < Math.min(index + lineNum, lines.length); i$2++) {
			highlightWordInLine.call(this, lines[i$2], comment$1, word, classActiveWord);
		}
		if (classActivePre) this.addClassToHast(this.pre, classActivePre);
		return true;
	}, options.matchAlgorithm);
...
function transformerRemoveComments(options = {}) {
	...
	return {
		...
		preprocess(_code, options2) {
			if (options2.includeExplanation !== true && options2.includeExplanation !== "scopeName") throw new Error("`transformerRemoveComments` requires `includeExplanation` to be set to `true` or `'scopeName'`");
		},
		tokens(tokens) {
			const result = [];
			for (const line of tokens) {
				...
				for (const token$1 of line) {
					const isComment = token$1.explanation?.some((exp) => exp.scopes.some((s$1) => s$1.scopeName.startsWith("comment")));
					if (isComment) {
						hasComment = true;
					} else {
						filteredLine.push(token$1);
					}
				}
				if (removeEmptyLines && hasComment) {
					const isAllWhitespace = filteredLine.every((token$1) => !token$1.content.trim());
		...
		code(hast) {
			...
			for (const line of hast.children) {
				...
				for (const [line2, level2] of emptyLines) {
					line2.children.unshift(...Array.from({ length: Math.min(ranges.length, level2 + 1) }, (_$2, i$2) => ({
						type: "element",
						tagName: "span",
						properties: {
							class: "indent",
...
function splitSpaces(parts, type, renderContinuousSpaces = true) {
	...
	let rightCount = 0;
	if (type === "boundary") {
		for (let i$2 = 0; i$2 < parts.length; i$2++) {
			if (isSpace(parts[i$2])) leftCount++;
			else break;
		}
	}
	if (type === "boundary" || type === "trailing") {
		for (let i$2 = parts.length - 1; i$2 >= 0; i$2--) {
			if (isSpace(parts[i$2])) rightCount++;
			else break;
		}
	}
...
function transformerRenderWhitespace(options = {}) {
	...
	return {
		...
		root(root$1) {
			...
			const code = pre.tagName === "pre" ? pre.children[0] : { children: [root$1] };
			code.children.forEach((line) => {
				...
				line.children = line.children.flatMap((token$1) => {
					...
					const parts = splitSpaces(node.value.split(/([ \t])/).filter((i$2) => i$2.length), position === "boundary" && index === last && last !== 0 ? "trailing" : position, position !== "trailing");
...
function renderDiffWithHighlighter(diff, highlighter$1, options, forcePlainText = false) {
	const baseThemeType = (() => {
		...
		if (typeof theme === "string") {
			return highlighter$1.getTheme(theme).type;
		}
		return undefined;
	})();
...
function computeLineDiffDecorations({ oldLine, newLine, oldLineIndex, newLineIndex, oldDecorations, newDecorations, lineDiffType }) {
	...
	const lineDiff$1 = lineDiffType === "char" ? diffChars(oldLine, newLine) : diffWordsWithSpace(oldLine, newLine);
	const deletionSpans = [];
	const additionSpans = [];
	const enableJoin = lineDiffType === "word-alt";
	for (const item of lineDiff$1) {
		const isLastItem = item === lineDiff$1[lineDiff$1.length - 1];
	...
	for (const span of deletionSpans) {
		if (span[0] === 1) {
			oldDecorations.push(createDiffSpanDecoration({
				line: oldLineIndex - 1,
				spanStart: spanIndex,
	...
	for (const span of additionSpans) {
		if (span[0] === 1) {
			newDecorations.push(createDiffSpanDecoration({
				line: newLineIndex - 1,
				spanStart: spanIndex,
				spanLength: span[1].length
			}));
