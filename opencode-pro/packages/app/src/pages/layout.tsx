...
export default function Layout(props: ParentProps) {
  ...
  const SidebarContent = (sidebarProps: { mobile?: boolean }) => {
    ...
    const openEditTaskDialog = () => {
      const task = scheduler.state.tasks.find((t) => t.id === scheduler.state.selectedTaskId)
      if (task) {
        dialog.show(() => <DialogCreateTask task={task} />)
      }
    }

    onMount(() => {
      if (scheduler.state.view === "scheduler") {
        scheduler.loadTasks()
      }
    })
          ...
          <div class="flex flex-col items-start self-stretch gap-4 px-2 overflow-hidden min-h-0">
            ...
            <Show when={!expanded()}>
              <div class="flex flex-col gap-1 w-full shrink-0">
                <Tooltip placement="right" value="Workspace">
                  <Button
                    variant="ghost"
                    size="large"
            ...
            <Switch>
              <Match when={scheduler.state.view === "workspace"}>
                <DragDropProvider
              ...
              <Match when={scheduler.state.view === "scheduler"}>
                <div class="w-full min-h-0 overflow-y-auto no-scrollbar">
                  <Show
                    when={!scheduler.state.selectedTaskId}
                    fallback={
                      <TaskDetail
                        onEdit={openEditTaskDialog}
                        onBack={() => scheduler.selectTask(null)}
                      />
                    }
                  >
