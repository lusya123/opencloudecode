...
export function getMouseProtocol(getMode: (mode: number, isAnsi?: boolean) => boolean): MouseProtocol {
  // xterm mouse reporting protocols are DEC private modes.
...
export function wheelEventToLineDelta(
  ...
  // Convert wheel delta into terminal rows, mirroring ghostty-web's logic.
  if (event.deltaMode === WheelEvent.DOM_DELTA_PIXEL) {
    return event.deltaY / options.lineHeight
  }
  if (event.deltaMode === WheelEvent.DOM_DELTA_LINE) {
    return event.deltaY
  }
  if (event.deltaMode === WheelEvent.DOM_DELTA_PAGE) {
    return event.deltaY * options.rows
  }
...
export function wheelEventToSteps(event: Pick<WheelEvent, "deltaMode" | "deltaY">, options: { lineHeight: number; rows: number }) {
  ...
  if (!delta) return 0
  // Map smooth deltas into discrete wheel steps for mouse reporting. Keep it bounded to avoid huge bursts.
...
export function encodeXtermMouseWheel(opts: {
  ...
  if (opts.protocol === "sgr") {
    return `\x1b[<${cb};${opts.col};${opts.row}M`
  }

  if (opts.protocol === "urxvt") {
    return `\x1b[${cb + 32};${opts.col};${opts.row}M`
  }

  // X10 mouse mode is limited to 223 for coordinates.
